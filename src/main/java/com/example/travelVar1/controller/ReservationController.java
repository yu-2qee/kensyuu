package com.example.travelVar1.controller;
...
import com.example.travelVar1.form.ReservationInputForm;
...
import com.example.travelVar1.security.UserDetailsImpl;
import com.example.travelVar1.service.ReservationService;
import com.example.travelVar1.service.StripeService;
...
public ReservationController(ReservationRepository reservationRepository,
		HouseRepository houseRepository,ReservationService reservationService,StripeService stripeService) {
...
@PageableDefault(page=0,sort="id",size=10,direction=Direction.ASC)Pageable pageable,Model model){
	...
	if(numberOfPeople !=null) {
		if(!reservationService.isWithinCapacity(numberOfPeople,capacity)) {
			FieldError fieldError=new FieldError
				(bindingResult.getObjectName(),"numberOfPeople","宿泊人数が定員を超えています");
...
public String confirm(@PathVariable(name="id")Integer id,
		...
		@AuthenticationPrincipal UserDetailsImpl userDetailsImpl,HttpServletRequest httpServletRequest,Model model) {
	House house= houseRepository.getReferenceById(id);
	User user=userDetailsImpl.getUser();
